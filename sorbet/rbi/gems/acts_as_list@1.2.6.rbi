# typed: true

# DO NOT EDIT MANUALLY
# This is an autogenerated file for types exported from the `acts_as_list` gem.
# Please instead update this file by running `bin/tapioca gem acts_as_list`.


# pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/with_connection.rb:3
module ActiveRecord; end

# pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/with_connection.rb:4
module ActiveRecord::Acts; end

# pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/with_connection.rb:5
module ActiveRecord::Acts::List; end

# pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/add_new_at_method_definer.rb:3
module ActiveRecord::Acts::List::AddNewAtMethodDefiner
  class << self
    # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/add_new_at_method_definer.rb:4
    def call(caller_class, add_new_at); end
  end
end

# pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/aux_method_definer.rb:3
module ActiveRecord::Acts::List::AuxMethodDefiner
  class << self
    # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/aux_method_definer.rb:4
    def call(caller_class); end
  end
end

# pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/callback_definer.rb:3
module ActiveRecord::Acts::List::CallbackDefiner
  class << self
    # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/callback_definer.rb:4
    def call(caller_class, add_new_at); end
  end
end

# pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:9
module ActiveRecord::Acts::List::ClassMethods
  # Configuration options are:
  #
  # * +column+ - specifies the column name to use for keeping the position integer (default: +position+)
  # * +scope+ - restricts what is to be considered a list. Given a symbol, it'll attach <tt>_id</tt>
  #   (if it hasn't already been added) and use that as the foreign key restriction. It's also possible
  #   to give it an entire string that is interpolated if you need a tighter scope than just a foreign key.
  #   Example: <tt>acts_as_list scope: 'todo_list_id = #{todo_list_id} AND completed = 0'</tt>
  # * +top_of_list+ - defines the integer used for the top of the list. Defaults to 1. Use 0 to make the collection
  #   act more like an array in its indexing.
  # * +add_new_at+ - specifies whether objects get added to the :top or :bottom of the list. (default: +bottom+)
  #                   `nil` will result in new items not being added to the list on create.
  # * +sequential_updates+ - specifies whether insert_at should update objects positions during shuffling
  #   one by one to respect position column unique not null constraint.
  #   Defaults to true if position column has unique index, otherwise false.
  #   If constraint is <tt>deferrable initially deferred<tt>, overriding it with false will speed up insert_at.
  # * +touch_on_update+ - configuration to disable the update of the model timestamps when the positions are updated.
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:26
  def acts_as_list(options = T.unsafe(nil)); end
end

# pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:74
module ActiveRecord::Acts::List::InstanceMethods
  # Get the current position of the item in the list
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:76
  def current_position; end

  # Decrease the position of this item without adjusting the rest of the list.
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:154
  def decrement_position; end

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:212
  def default_position; end

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:216
  def default_position?; end

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:159
  def first?; end

  # Return the next higher item in the list.
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:170
  def higher_item; end

  # Return the next n higher items in the list
  # selects all higher items by default
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:177
  def higher_items(limit = T.unsafe(nil)); end

  # Test if this record is in a list
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:204
  def in_list?; end

  # Increase the position of this item without adjusting the rest of the list.
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:148
  def increment_position; end

  # Insert the item at the given position (defaults to the top position of 1).
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:82
  def insert_at(position = T.unsafe(nil)); end

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:86
  def insert_at!(position = T.unsafe(nil)); end

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:164
  def last?; end

  # Return the next lower item in the list.
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:187
  def lower_item; end

  # Return the next n lower items in the list
  # selects all lower items by default
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:194
  def lower_items(limit = T.unsafe(nil)); end

  # Swap positions with the next higher item, if one exists.
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:105
  def move_higher; end

  # Swap positions with the next lower item, if one exists.
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:91
  def move_lower; end

  # Move to the bottom of the list. If the item is already in the list, the items below it have their
  # position adjusted accordingly.
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:120
  def move_to_bottom; end

  # Move to the top of the list. If the item is already in the list, the items above it have their
  # position adjusted accordingly.
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:127
  def move_to_top; end

  # Move the item within scope. If a position within the new scope isn't supplied, the item will
  # be appended to the end of the list.
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:142
  def move_within_scope(scope_id); end

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:208
  def not_in_list?; end

  # Removes the item from the list.
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:133
  def remove_from_list; end

  # Sets the new position and saves it
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:221
  def set_list_position(new_position, raise_exception_if_save_fails = T.unsafe(nil)); end

  private

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:487
  def active_record_version_is?(version_requirement); end

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:235
  def acts_as_list_list; end

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:483
  def acts_as_list_order_argument(direction = T.unsafe(nil)); end

  # Forces item to assume the bottom position in the list.
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:290
  def assume_bottom_position; end

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:262
  def assume_default_position?; end

  # Forces item to assume the top position in the list.
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:295
  def assume_top_position; end

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:239
  def avoid_collision; end

  # Returns the bottom item
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:279
  def bottom_item(except = T.unsafe(nil)); end

  # Returns the bottom position number in the list.
  #   bottom_position_in_list    # => 2
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:273
  def bottom_position_in_list(except = T.unsafe(nil)); end

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:447
  def check_scope; end

  # This check is skipped if the position is currently the default position from the table
  # as modifying the default position on creation is handled elsewhere
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:461
  def check_top_position; end

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:443
  def clear_scope_changed; end

  # This has the effect of moving all the higher items up one.
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:321
  def decrement_positions_on_higher_items(position); end

  # This has the effect of moving all the lower items up one.
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:326
  def decrement_positions_on_lower_items(position = T.unsafe(nil)); end

  # Increments position (<tt>position_column</tt>) of all items in the list.
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:337
  def increment_positions_on_all_items; end

  # This has the effect of moving all the higher items down one.
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:300
  def increment_positions_on_higher_items; end

  # This has the effect of moving all the lower items down one.
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:306
  def increment_positions_on_lower_items(position, avoid_id = T.unsafe(nil)); end

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:389
  def insert_at_position(position, raise_exception_if_save_fails = T.unsafe(nil)); end

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:437
  def internal_scope_changed?; end

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:429
  def position_before_save; end

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:421
  def position_before_save_changed?; end

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:493
  def primary_key_condition(id = T.unsafe(nil)); end

  # When using raw column name it must be quoted otherwise it can raise syntax errors with SQL keywords (e.g. order)
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:468
  def quoted_position_column; end

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:479
  def quoted_position_column_with_table_name; end

  # Used in order clauses
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:475
  def quoted_table_name; end

  # Overwrite this method to define the scope of the list changes
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:269
  def scope_condition; end

  # Reorders intermediate items to support moving an item from old_position to new_position.
  # unique constraint prevents regular increment_all and forces to do increments one by one
  # http://stackoverflow.com/questions/7703196/sqlite-increment-unique-integer-field
  # both SQLite and PostgreSQL (and most probably MySQL too) has same issue
  # that's why *sequential_updates?* check alters implementation behavior
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:346
  def shuffle_positions_on_intermediate_items(old_position, new_position, avoid_id = T.unsafe(nil)); end

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:228
  def swap_positions_with(item); end

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/list.rb:409
  def update_positions; end
end

# pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/no_update.rb:6
module ActiveRecord::Acts::List::NoUpdate
  mixes_in_class_methods ::ActiveRecord::Acts::List::NoUpdate::ClassMethods

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/no_update.rb:119
  def act_as_list_no_update?; end

  class << self
    # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/no_update.rb:98
    def applied_to?(klass); end

    # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/no_update.rb:91
    def apply_to(klasses); end

    # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/no_update.rb:8
    def included(base); end

    private

    # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/no_update.rb:108
    def add_klass(klass); end

    # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/no_update.rb:104
    def extracted_klasses; end

    # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/no_update.rb:113
    def remove_klass(klass); end
  end
end

# pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/no_update.rb:12
class ActiveRecord::Acts::List::NoUpdate::ArrayTypeError < ::ArgumentError
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/no_update.rb:13
  def initialize; end
end

# pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/no_update.rb:24
module ActiveRecord::Acts::List::NoUpdate::ClassMethods
  # Lets you selectively disable all act_as_list database updates
  # for the duration of a block.
  #
  # ==== Examples
  #
  # class TodoList < ActiveRecord::Base
  #   has_many :todo_items, -> { order(position: :asc) }
  # end
  #
  # class TodoItem < ActiveRecord::Base
  #   belongs_to :todo_list
  #
  #   acts_as_list scope: :todo_list
  # end
  #
  # TodoItem.acts_as_list_no_update do
  #   TodoList.first.update(position: 2)
  # end
  #
  # You can also pass an array of classes as an argument to disable database updates on just those classes.
  # It can be any ActiveRecord class that has acts_as_list enabled.
  #
  # ==== Examples
  #
  # class TodoList < ActiveRecord::Base
  #   has_many :todo_items, -> { order(position: :asc) }
  #   acts_as_list
  # end
  #
  # class TodoItem < ActiveRecord::Base
  #   belongs_to :todo_list
  #   has_many :todo_attachments, -> { order(position: :asc) }
  #
  #   acts_as_list scope: :todo_list
  # end
  #
  # class TodoAttachment < ActiveRecord::Base
  #   belongs_to :todo_list
  #   acts_as_list scope: :todo_item
  # end
  #
  # TodoItem.acts_as_list_no_update([TodoAttachment]) do
  #   TodoItem.find(10).update(position: 2)
  #   TodoAttachment.find(10).update(position: 1)
  #   TodoAttachment.find(11).update(position: 2)
  #   TodoList.find(2).update(position: 3) # For this instance the callbacks will be called because we haven't passed the class as an argument
  # end
  #
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/no_update.rb:73
  def acts_as_list_no_update(extra_classes = T.unsafe(nil), &block); end

  private

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/no_update.rb:85
  def active_record_objects?(extra_classes); end
end

# pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/no_update.rb:18
class ActiveRecord::Acts::List::NoUpdate::DisparityClassesError < ::ArgumentError
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/no_update.rb:19
  def initialize; end
end

# pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/position_column_method_definer.rb:5
module ActiveRecord::Acts::List::PositionColumnMethodDefiner
  class << self
    # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/position_column_method_definer.rb:6
    def call(caller_class, position_column, touch_on_update); end

    # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/position_column_method_definer.rb:17
    def define_class_methods(caller_class, position_column, touch_on_update); end

    # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/position_column_method_definer.rb:62
    def define_instance_methods(caller_class, position_column); end

    # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/position_column_method_definer.rb:99
    def mass_assignment_protection_was_used_by_user?(caller_class); end

    # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/position_column_method_definer.rb:105
    def protect_attributes_from_mass_assignment(caller_class, position_column); end
  end
end

# pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/scope_method_definer.rb:4
module ActiveRecord::Acts::List::ScopeMethodDefiner
  class << self
    # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/scope_method_definer.rb:6
    def call(caller_class, scope); end

    # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/scope_method_definer.rb:85
    def idify(caller_class, name); end
  end
end

# pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/sequential_updates_method_definer.rb:5
module ActiveRecord::Acts::List::SequentialUpdatesMethodDefiner
  class << self
    # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/sequential_updates_method_definer.rb:6
    def call(caller_class, column, sequential_updates_option); end
  end
end

# pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/top_of_list_method_definer.rb:3
module ActiveRecord::Acts::List::TopOfListMethodDefiner
  class << self
    # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/top_of_list_method_definer.rb:4
    def call(caller_class, top_of_list); end
  end
end

# pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/with_connection.rb:6
class ActiveRecord::Acts::List::WithConnection
  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/with_connection.rb:7
  def initialize(recipient); end

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/with_connection.rb:13
  def call; end

  # pkg:gem/acts_as_list#lib/acts_as_list/active_record/acts/with_connection.rb:11
  def recipient; end
end
